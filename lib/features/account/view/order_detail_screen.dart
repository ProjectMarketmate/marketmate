...
class OrderDetailScreen extends StatefulWidget {
  const OrderDetailScreen(
      {super.key, required this.order, required this.getOrders});
  final Order order;
...
class _OrderDetailScreenState extends State<OrderDetailScreen> {
  ...
  void cancelOrders() async {
    try {
      final resp = await dioClient.patch('/app/order/${widget.order.id}/',
      ...
      body: Padding(
        ...
        child: Stack(
          children: [
            ...
            if (widget.order.status != ORDER_STATUS.CANCELED &&
                widget.order.status != ORDER_STATUS.DELIVERED)
              Positioned(
                  bottom: 25,
                  left: 5,
                  right: 5,
